...
# Developer mods from GitHub ("Моды игры" tab)
github_developer_mods = []
last_github_fetch_error = ""
...
def _github_headers():
    # Public repos usually work without auth; keep headers minimal.
    return {
        "Accept": "application/vnd.github+json",
        "User-Agent": "FNAF2-Mods-Client"
    }

def fetch_github_mods_list(repo_full_name=GITHUB_DEVELOPER_MODS_REPO, branch=GITHUB_DEVELOPER_MODS_BRANCH, timeout=10):
    """Returns list of dicts: {name, filename, download_url, size}. Only .py files from repo root.

    Note: We intentionally keep it simple (root only) to match the user's workflow
    (create a new .py in repo → it shows up in 'Моды игры').
    """
...
def download_github_mod(mod, target_dir=mods_dir, timeout=20):
    """Download a single mod (dict returned by fetch_github_mods_list) into mods_dir."""
    try:
...
def get_flashlight_capacity_ms(current_night, is_custom_night, custom_levels):
    if is_custom_night:
        # If ALL animatronics AI == 20 → 50 seconds.
        all20 = all(int(v) == 20 for v in custom_levels.values()) if custom_levels else False
        return int((FLASHLIGHT_CUSTOM_ALL20_SEC if all20 else FLASHLIGHT_CUSTOM_DEFAULT_SEC) * 1000)
    return int(FLASHLIGHT_CAPACITY_BY_NIGHT_SEC.get(int(current_night), 110) * 1000)

def get_flashlight_icon_key(percent):
...
# --- Flashlight (battery) HUD ---
# These images should be placed in the game's `images/` folder:
# FlFull.png, FlThree.png, FlTwo.png, FlOne.png, FlZero.png
FLASH_ICON_SIZE = (max(48, int(WIN_H * 0.08)), max(48, int(WIN_H * 0.08)))

# Flashlight capacity (continuous use) per night in seconds
FLASHLIGHT_CAPACITY_BY_NIGHT_SEC = {
    1: 110,  # 1:50
    2: 110,
    3: 100,  # 1:40
    4: 90,   # 1:30
...
FLASHLIGHT_CUSTOM_DEFAULT_SEC = 110
FLASHLIGHT_CUSTOM_ALL20_SEC = 50

# --- Developer mods (GitHub) ---
GITHUB_DEVELOPER_MODS_REPO = "DenisGadasin/FNAF2-mods"
GITHUB_DEVELOPER_MODS_BRANCH = "main"
...
imgs = {
    ...
    # Flashlight battery icons (HUD)
    "fl_full": load_img("FlFull.png", FLASH_ICON_SIZE),
    "fl_three": load_img("FlThree.png", FLASH_ICON_SIZE),
    "fl_two": load_img("FlTwo.png", FLASH_ICON_SIZE),
    "fl_one": load_img("FlOne.png", FLASH_ICON_SIZE),
    "fl_zero": load_img("FlZero.png", FLASH_ICON_SIZE),
        ...
        if self.name == "Withered Foxy":
            if self.pos == "Parts Service":
                ...
                if not is_watching_hall:
                    ...
                    # User request: increase attack countdown from 7.5s to 10s
                    self.status_msg = f"Attack in: {10.0 - self.foxy_unwatched_time/1000:.1f}s"
                    if self.foxy_unwatched_time >= 10000:
                        self.pos = "Office"
                else:
                    self.foxy_watched_time += dt
                    self.foxy_unwatched_time = max(0, self.foxy_unwatched_time - dt)
                    # User request: to send Foxy away, keep flashlight on him for 7.5s
                    self.status_msg = f"Blinding: {7.5 - self.foxy_watched_time/1000:.1f}s"
                    if self.foxy_watched_time >= 7500:
                        self.pos = self.start_pos
                        self.foxy_watched_time = 0
            return
...
def reset_game():
    ...
    # Reset flashlight battery per-night
    flashlight_capacity_ms = get_flashlight_capacity_ms(current_night, is_custom_night, custom_levels)
    flashlight_remaining_ms = flashlight_capacity_ms
    flashlight_percent = 100
    flashlight_last_tick = pygame.time.get_ticks()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if game_state in ["PLAY", "CHECKING"]:
                ...
                # Developer mods (GitHub) interactions
                if current_mods_tab == "developer":
                    # refresh button
                    if pygame.Rect(WIN_W // 2 - 400 + 150 + 20 + 10, WIN_H // 2 - 300 + 80 + 10, 170, 40).collidepoint(mouse_pos):
                        try:
                            github_developer_mods = fetch_github_mods_list()
                            last_github_fetch_error = "" if github_developer_mods else "Не удалось получить список модов (проверь интернет/репозиторий)"
                        except Exception:
                            github_developer_mods = []
                            last_github_fetch_error = "Ошибка запроса к GitHub"

                    # download buttons
                    if github_developer_mods:
                        content_x = (WIN_W // 2 - 400) + 150 + 20
                        content_y = (WIN_H // 2 - 300) + 80
                        for i, mod in enumerate(github_developer_mods[:6]):
                            mod_y = content_y + 70 + i * 80
                            download_btn = pygame.Rect(content_x + 240, mod_y + 10, 140, 40)
                            if download_btn.collidepoint(mouse_pos):
                                ok = download_github_mod(mod)
                                if ok:
                                    scan_mods()  # refresh library
                                    last_github_fetch_error = "Скачано! (см. вкладку 'Библиотека')"
                                else:
                                    last_github_fetch_error = "Не удалось скачать мод"
    ...
    if game_state in ["PLAY", "CHECKING"]:
        ...
        # Flashlight usage:
        # - In cameras: hold LCTRL (existing behaviour: light_on)
        # - In office: hold Z/X/C to view hall/vents (also consumes battery)
        light_on = keys[pygame.K_LCTRL] and camera_mode

        # Determine if flashlight is being used (consumes battery)
        using_flashlight = False
        if camera_mode and light_on:
            using_flashlight = True
        if (show_hall or show_vent_l or show_vent_r):
            using_flashlight = True

        # Consume battery while flashlight is used; disable flashlight when empty
        now_tick = pygame.time.get_ticks()
        if flashlight_last_tick == 0:
            flashlight_last_tick = now_tick

        if using_flashlight and flashlight_remaining_ms > 0:
            elapsed = now_tick - flashlight_last_tick
            if elapsed > 0:
                flashlight_remaining_ms = max(0, flashlight_remaining_ms - elapsed)
        flashlight_last_tick = now_tick

        flashlight_capacity_ms = flashlight_capacity_ms or get_flashlight_capacity_ms(current_night, is_custom_night, custom_levels)
        flashlight_percent = int(round((flashlight_remaining_ms / max(1, flashlight_capacity_ms)) * 100))

        if flashlight_remaining_ms <= 0:
            # Force-disable flashlight effects when empty
            light_on = False
            show_hall = False
            show_vent_l = False
            show_vent_r = False
        ...
        # HUD: Night counter + time
        display_night = 7 if is_custom_night else current_night
        screen.blit(font_clock.render(f"Night {display_night}", True, (255, 255, 255)), (WIN_W - 220, 25))
        screen.blit(font_clock.render(f"{hour} AM", True, (255, 255, 255)), (WIN_W - 150, 70))

        # Flashlight battery icon (top-left)
        fl_icon_key = get_flashlight_icon_key(flashlight_percent)
        fl_icon = imgs.get(fl_icon_key)
        if fl_icon:
            screen.blit(fl_icon, (20, 20))
        ...
        # Developer mods buttons (GitHub)
        # (content_x/content_y are defined below, so we create rect after that)
        ...
        if current_mods_tab == "popular":
            empty_text = font_mods.render("Популярные моды скоро появятся!", True, (200, 200, 200))
            screen.blit(empty_text, (content_x + 50, content_y + 100))

        elif current_mods_tab == "developer":
            # GitHub developer mods list
            pygame.draw.rect(screen, (70, 130, 180), refresh_developer_btn, border_radius=10)
            pygame.draw.rect(screen, (200, 200, 200), refresh_developer_btn, 2, border_radius=10)
            screen.blit(font_bug_report.render("Обновить", True, (255, 255, 255)),
                        (refresh_developer_btn.x + 45, refresh_developer_btn.y + 10))

            if last_github_fetch_error:
                err = font_bug_input.render(last_github_fetch_error, True, (255, 120, 120))
                screen.blit(err, (content_x + 10, content_y + 60))

            if not github_developer_mods:
                empty_text = font_mods.render("Модов от игры пока нет.", True, (200, 200, 200))

        # Define content area after tabs
        content_x = window_x + panel_width + 20
        content_y = window_y + 80

        refresh_developer_btn = pygame.Rect(content_x + 10, content_y + 10, 170, 40)

                screen.blit(empty_text, (content_x + 10, content_y + 120))
                hint = font_bug_input.render("Нажмите 'Обновить' для загрузки списка с GitHub", True, (150, 150, 150))
                screen.blit(hint, (content_x + 10, content_y + 150))
            else:
                for i, mod in enumerate(github_developer_mods[:6]):
                    mod_y = content_y + 70 + i * 80
                    screen.blit(font_mods.render(mod['name'], True, (255, 255, 255)), (content_x + 10, mod_y))
                    download_btn = pygame.Rect(content_x + 240, mod_y + 10, 140, 40)
                    pygame.draw.rect(screen, (100, 100, 150), download_btn)
                    pygame.draw.rect(screen, (200, 200, 200), download_btn, 2)
                    screen.blit(font_bug_report.render("Скачать", True, (255, 255, 255)),
                                (download_btn.x + 35, download_btn.y + 10))
